...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	{		
		if (isRepaintImg)
		{
			if ( (GetTickCount() - cur_tick) > 30*1000 )
			{
				PaintBk(pDC);
				cur_tick = GetTickCount();
				_RPTF0(_CRT_WARN,"Repaint ===> \n");
			}
			TRACE("===== > Paint Transparent <=====\n");
			DrawTransparent();
		}
		else
		{
...
{	
	if ( m_bisMyBk )
	{
		...
		if (m_dcBk.m_hDC == NULL)
		{
			...
			HBITMAP m_hBitmap=(HBITMAP)LoadImage(GetModuleHandle(NULL), m_sMyBkFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			//m_bmpBk.CreateCompatibleBitmap(&clDC, rect.Width(), rect.Height());
			m_bmpBk.Attach(m_hBitmap);
			m_pbmpOldBk = m_dcBk.SelectObject(&m_bmpBk);
			//m_dcBk.BitBlt(rect.left, rect.top, rect.Width(), rect.Height(), &clDC, rect1.left, rect1.top, SRCCOPY);
		} // if

		pDC->BitBlt(rect.left, rect.top, rect.Width(), rect.Height(), &m_dcBk,rect1.left, rect1.top, SRCCOPY);
	}
	else
	{
		CClientDC clDC(GetParent());
...
void CButtonST::SetRepaingImg(BOOL bPaint)
{
	isRepaintImg = bPaint;
}
...
void CButtonST::DrawTransparent(BOOL bRepaint)
{
	...
	{
		m_dcBk.SelectObject(m_pbmpOldBk);
		TRACE("===== > Paint Transparent Select OBJ<=====\n");
	} // if
